...
def get_recent_player_minutes(days):
...
def get_current_data(days):
    
    """ pulls raptor ratings and player minutes from the web
    returns dfr_current, dfp_current"""
    
    dfr_current = get_latest_raptor()
    dfp_current = get_recent_player_minutes(days)
...
def predict_todays_games(dfp_adj, dfr_current, model, prob_model, features, 
    ...
    hour = now.strftime("%H")
    df_games_csv.to_csv('game_predictions_{0}_{1}_{2}_{3}.csv'.format(year,smonth,sday,hour))
...
def run_prediction_script():
    ...
    #yes, some of these constants are a bit magical and seamingly arbitrary
    ...
    dfr_current, dfp_current = get_current_data(200)
    #dfr_current = get_latest_raptor()
    #dfp_current_tep = pd.read_csv('dfp_current.cvs')
    #dfp_current = dfp_current_tep.iloc[:,1:]
